...
class Tester(object):
    ...
    def render_multiview(self, n_views=12):
        ...
        for i, pathname in enumerate(pathnames):
            ...
            for angle in range(0,360,self.args.angle_interval):
                ...
                img, mask, others = self.model.render(pred_tex,pred_shape, attn_map=None, rotation=torch.tensor([quaternion]).cuda(), \
                                                      scale=scale, translation=translation, novel_view=True)
                if angle==0:
                    mesh_dir = os.path.join('./outputs/mesh',self.args.dataset)
                    #print(pred_tex)
                    #print(pred_tex.shape)
                    os.makedirs(mesh_dir, exist_ok=True)
                    save_dir = os.path.join(mesh_dir,str(idx))
                    self.model.mesh_template.export_obj(mesh_dir, others['vtx_3d'][0], pred_tex[0])
    ...
    def eval_fid(self):
        ...
        if self.args.eval_option == 'FID_1':
            ...
            emb_fake = []
            emb_real = []
            inversion_path_text = open(os.path.join(self.args.data_dir,'cache','inversion_path.txt'),'w')
            for data in data_loader:
                for idx, path in zip(data['idx'].numpy(), data['img_path']):
                    inversion_path_text.write(str(idx))
                    inversion_path_text.write(' ')
                    inversion_path_text.write(path)
                    inversion_path_text.write('\n')
            inversion_path_text.close()
            ...
            fid_test_save_path = os.path.join(self.args.data_dir,'cache', f'precomputed_fid_299x299_testval.npz')

            
            np.savez_compressed(fid_test_save_path, 
                                stats_m=m2,
                                stats_s=np.tril(s2.astype(np.float32)), # The matrix is symmetric, we only store half of it
                                num_images=len(emb_real),
                                resolution=299,
                               )
        else:
            ...
            test_path = os.path.join(self.args.data_dir)
            data_loader = torch.utils.data.DataLoader(data_set, batch_size=40, num_workers=8, \
                        pin_memory=True, drop_last=False, shuffle=False)
            
            # load_gt_stats
            filepath = os.path.join(self.args.data_dir, 'cache', 'precomputed_fid_299x299_testval.npz')
