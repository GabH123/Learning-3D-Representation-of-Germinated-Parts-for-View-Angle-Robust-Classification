...
class BasePseudoDataset(torch.utils.data.Dataset):
    def __init__(self, args, augment=True):
        ...
        self.data = np.load(os.path.join(self.data_dir, 'cache', 'poses_metadata.npz'), allow_pickle=True)
        self.data = self.data['data'].item()
        num_images = len(self.data['path'])
        self.augment = augment
        pseudogt_files = glob.glob(os.path.join(self.data_dir,
    ...
    def _load_pseudogt(self, idx):
        ...
        data = np.load(os.path.join(self.data_dir,
...
class PseudoDatasetForEvaluation(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset.args.conditional_encoding:
            gt_dict['encoding'] = self.dataset.feat[idx]
